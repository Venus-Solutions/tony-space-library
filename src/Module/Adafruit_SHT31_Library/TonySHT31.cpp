 ...
 *         optional wire
 */
TonySHT31::TonySHT31(TwoWire *theWire) {
  _wire = theWire;
  _i2caddr = NULL;
  humidity = 0.0f;
  temp = 0.0f;
...
boolean TonySHT31::begin(uint8_t i2caddr) {
  _wire->begin();
  _i2caddr = i2caddr;
  reset();
...
uint16_t TonySHT31::readStatus(void) {
...
void TonySHT31::reset(void) {
  writeCommand(SHT31_SOFTRESET);
  delay(10);
}

void TonySHT31::heater(boolean h) {
  if (h)
...
float TonySHT31::readTemperature(void) {
  if (!readTempHum())
    return NAN;
...
float TonySHT31::readHumidity(void) {
  if (!readTempHum())
    return NAN;
...
boolean TonySHT31::readTempHum(void) {
...
void TonySHT31::writeCommand(uint16_t cmd) {
...
uint8_t TonySHT31::crc8(const uint8_t *data, int len) {
  /*
   *
